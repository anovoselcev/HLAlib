...
throw (FederateInternalError){
    ...
    if(theInteraction == _InteractionClasses[L"READY"] && _state >= STATE::STARTED){
        lock_guard<mutex> guard(_smutex);
        size_t hash = 0;
        if(theUserSuppliedTag.size() == 4){
            hash = HLA::cast_from_rti<HLA::Unsigned32BE>(theUserSuppliedTag);
            if(!_federates_hash[static_cast<size_t>(hash)].isValid()){
                hash = HLA::cast_from_rti<HLA::Unsigned32LE>(theUserSuppliedTag);
            }
        }
        if(theUserSuppliedTag.size() == 8){
            hash = HLA::cast_from_rti<HLA::Unsigned64BE>(theUserSuppliedTag);
            if(!_federates_hash[static_cast<size_t>(hash)].isValid()){
                hash = HLA::cast_from_rti<HLA::Unsigned64LE>(theUserSuppliedTag);
            }
        }
        else throw std::runtime_error("Bad READY");
