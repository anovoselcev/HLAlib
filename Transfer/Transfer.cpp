...
class RTIButton : public HLA::BaseRTIFixedRecord<Button,8>{
public:
    void getDataMax(void *ptrSource, unsigned long uiMaxSize){
        HLA::RTIinteger32LE field1;
...
class RTITumbler : public HLA::BaseRTIFixedRecord<Tumbler,8>{
public:
    void getDataMax(void *ptrSource, unsigned long uiMaxSize){
        HLA::RTIinteger32LE field1;
        HLA::RTIstring field2;
...
//          _rtiAmbassador->updateAttributeValues(_MyInstanceID,m,VariableLengthData());
    ...
    void Transfer::ParameterProcess(){
        ...
        while(!_qParameters.empty()){
            ...
            Button b = HLA::cast_from_rti<RTIButton>(message.find(_ParametersMap.at(name).at(L"Button"))->second);
...
//             std::wcout << a[0] << " " << a[33] << std::endl;
    ...
    void Transfer::receiveInteraction (
