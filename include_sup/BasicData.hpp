...
#include "include_sup/BasicConverter.hpp"
  ...
  namespace HLA {
    ...
    using unicodeString = std::vector<unicodeChar>;
    ...
    class ptrRtiAmb_class {
    public:

      void SetPtrToRtiAmb(rti1516e::RTIambassador* ptrRtiAmb) {
        m_ptrRtiAmb = ptrRtiAmb;
      }
    protected:
      static rti1516e::RTIambassador* m_ptrRtiAmb;
    };


    template <class T_FOM, class T_MOD, unsigned m_OBV=1>
    class RTIvariableArray : public ClassForRTI<std::vector<T_MOD>,m_OBV>
    {
    ...
    public:
      RTIvariableArray(const RTIvariableArray &variableArray):ClassForRTI<std::vector<T_MOD>,m_OBV>(variableArray) {}
      ...
      RTIvariableArray(std::vector<T_MOD> &inData) {
        ptrData = nullptr;
        m_uiSizeData = 0;
        get(inData);
      }
